...
def test_upload_track(track_service, sample_gpx):
    result = track_service.upload_track("test.gpx", sample_gpx, "test-user-id")
...
def test_duplicate_detection(track_service, sample_gpx):
    track_service.upload_track("test.gpx", sample_gpx, "test-user-id")

    result = track_service.upload_track("test2.gpx", sample_gpx, "test-user-id")
...
def test_list_tracks(track_service, sample_gpx):
    track_service.upload_track("track1.gpx", sample_gpx, "test-user-id")

    tracks = track_service.list_tracks("test-user-id")
...
def test_get_track_geometry(track_service, sample_gpx):
    ...
    geometry = track_service.get_track_geometry(track_id, "test-user-id")
...
def test_get_multiple_geometries(track_service):
    ...
    result1 = track_service.upload_track("track1.gpx", content1, "test-user-id")
    result2 = track_service.upload_track("track2.gpx", content2, "test-user-id")

    track_ids = [result1.track.id, result2.track.id]
    geometries = track_service.get_multiple_geometries(track_ids, "test-user-id")
...
def test_update_track_visibility(track_service, sample_gpx):
    ...
    updated = track_service.update_track(track_id, {"visible": False}, "test-user-id")
...
def test_update_nonexistent_track(track_service):
    result = track_service.update_track(9999, {"visible": False}, "test-user-id")
...
def test_delete_single_track(track_service, sample_gpx, tmp_path):
    ...
    gpx_file_path = tmp_path / "gpx" / f"test-user-id_{gpx_hash}.gpx"
    assert gpx_file_path.exists()

    delete_result = track_service.delete_tracks([track_id], "test-user-id")

    assert delete_result["deleted"] == 1
    assert delete_result["failed"] == 0
    assert len(delete_result["errors"]) == 0

    assert track_service.get_track_metadata(track_id, "test-user-id") is None
...
def test_delete_multiple_tracks(track_service, tmp_path):
    ...
    delete_result = track_service.delete_tracks(track_ids, "test-user-id")

    assert delete_result["deleted"] == 2
    assert delete_result["failed"] == 0
    assert len(delete_result["errors"]) == 0

    assert track_service.get_track_metadata(result1.track.id, "test-user-id") is None
    assert track_service.get_track_metadata(result2.track.id, "test-user-id") is None


def test_delete_nonexistent_track(track_service):
    delete_result = track_service.delete_tracks([9999], "test-user-id")
...
def test_delete_continues_on_failure(track_service, sample_gpx):
    result1 = track_service.upload_track("track1.gpx", sample_gpx, "test-user-id")
    track_id = result1.track.id

    delete_result = track_service.delete_tracks([9999, track_id, 8888], "test-user-id")
...
def test_delete_removes_from_spatial_table(track_service, sample_gpx):
    ...
    track_service.delete_tracks([track_id], "test-user-id")
